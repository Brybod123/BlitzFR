...
function createModelForFile(name, content) {
    const lang = name.endsWith('.html') ? 'html' : name.endsWith('.css') ? 'css' : 'javascript';
    return {
        model: monaco.editor.createModel(content, lang, monaco.Uri.file(name)),
        lang
    };
}

function resetEditorToTemplate(templateKey) {
    ...
    files = {
        'index.html': createModelForFile('index.html', template.html),
        'styles.css': createModelForFile('styles.css', template.css),
        'script.js': createModelForFile('script.js', template.js)
    };
    activeFile = 'index.html';
    editor.setModel(files[activeFile].model);
    currentProjectId = null;
    currentProjectOwnerUid = null;
    currentHostedWebsite = null;
    currentHostedSlug = null;
    forkSourceProject = null;
...
const initialHtml = starterTemplates.default.html;
const initialCss = starterTemplates.default.css;
const initialJs = starterTemplates.default.js;
...
const btnCreateBlank = document.getElementById('btn-create-blank');
const btnOpenTemplateMenu = document.getElementById('btn-open-template-menu');
const templateMenu = document.getElementById('template-menu');

btnCreateBlank?.addEventListener('click', () => resetEditorToTemplate('blank'));
btnOpenTemplateMenu?.addEventListener('click', (e) => {
    e.stopPropagation();
    templateMenu?.classList.toggle('hidden');
});

templateMenu?.querySelectorAll('.template-menu-item').forEach((btn) => {
    btn.addEventListener('click', () => {
        resetEditorToTemplate(btn.dataset.template);
